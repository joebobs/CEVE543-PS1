---
title: "Nonstationary Rainfall Frequency Analysis"
...
### Implement Bayesian GEV inference using MCMC with Turing.jl; specify and justify physically-informed priors for location, scale, and shape parameters based on Houston climate knowledge (see Lab 4 for multiple approaches - pick and justify one)
Re-initialize the gev model
```{julia}
#| output: false
@model function gev_model(y)
    μ ~ Normal(3.0, 3)
    log_σ ~ Normal(0, 1.0)  # <1>
    ξ ~ Normal(0.0, 0.3)
    σ = exp(log_σ)  # <2>
...
### Compare posterior distributions to MLE results and ensure convergence (see Lab 4)
Confirm convergence
```{julia}
let
...
*The covariate chosen was CO2 levels in the area. The intuition for that was because it is often found to be a proxy for large-scale climate and environmental changes. It also strongly correlates with global warming and hydrological cycles. Location (μ) and Scale (σ) are likely to vary with the covariate as location parameter represents the central tendancy of rainfall and an increasing CO2 can cause correspoding increase in rainfall. Scale parameter represents the spread of the extremes and the CO2 induced changes to the climate could contribute to that. The observation from the results here are that for the area selected, the affect of CO2 is not significant. (location and scale show covariate coefficients are not high and its confidence interval contains Zero)*
...
```
*The parameters that were chosen to be common across regional stations was shape and the covariate parameter for scale. This is because in the same region, while the effect of covariate may be similar (same covariate parameter), and the shape may be similar, the overall centrality (location) and the spread (scale) vary region to region based on minute changes in topology/climate related features of the area.*
...
*The 4 additional stations chosen were the closest stations to the original station (Sugarland) and are all within a 16km radius of it. This geographical proximity, while also being in the Gulf coast's subtropical climate, mean a similar exposure to storm events, atmospheric rivers and convective rainfall effects. The data length is also reasonable for the primary station > 100, and helps stabilize the regional analysis calculations for the other stations that do not have as many data points (>59)*
...
*While the 50 years of local data may seem adequate, the results above demonstrate that the Bayesian models significantly reduce the return period uncertainities using parameter sharing. The ability to use priors is also an exceptional utility as it allows the expert climate scientist to use information known about a region as prior information that the model can ground itself on. To support this, looking at the stations like 683 (THOMPSONS 3 WSW) with 59 data points shows that the regional estimation has significantly improved the uncertainity compared to the single stations estimation of return periods. The single-station Bayesian model had to estimate all parameters from one dataset alone, resulting in wider posteriors despite using Multi Chain Monte Carlo algorithms. The prior aware parameter sharing model isn't added complexity; it's more efficient use of available data and distributions, producing more precise parameter estimates and narrower uncertainty bounds for design values.

Assuming of stationarity adds risk that may go unnoticed in traditional analyses. The Mann-Kendall tests across the five stations show mixed signals (p-values ranging 0.18-0.97), which hint at a "no clear trend" conclusion. However, the nonstationary models reveal important patterns: the location parameter trend with CO2 (β_μ = 0.29-0.32 inches per log(ppm)) indicates potential intensification, even if the uncertainity implies that it may not be statistically significant. Further, the 1950 vs 2025 return level comparison shows the posterior mean shifts slightly upward. Projects in infrastructure design that use stationary 50-year estimates would thus likely underestimate flood risk if these trends continue. Also, due to many such prohjects having lifespans often exceeding 50-100 years and replacement costs that are enormous, the "complexity" of nonstationarity modeling is actually mitigating risk.*
